# Rachunek Macierzowy, prog. 5: Octave (solution.m) -> Python (15 wykresow) -> LaTeX (latexmk).
# Uruchom z katalogu prog_5/:  .\generate_ALL.ps1
...
$plotPy = Join-Path $Root "analysis\plot_prog5.py"
...
if (Test-Path $plotPy) {
  Write-Host "== Python: plot_prog5.py -> LaTeX/figures/ ==" -ForegroundColor Cyan
  Set-Location $Root
  & python $plotPy
  if ($LASTEXITCODE -ne 0) { Write-Error "Python zakonczyl sie kodem $LASTEXITCODE"; exit $LASTEXITCODE }
} else {
  Write-Host "== Python: brak analysis/plot_prog5.py - pomijam ==" -ForegroundColor Yellow
}
