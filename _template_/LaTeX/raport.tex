...
%   • faktyczne wyniki uruchomienia (przede wszystkim tabele; ewentualnie krótki, zredagowany
%     fragment tekstu — nie pełny zrzut konsoli ani ślad wywołań),
...
\subsection{Wyjście konsoli (opcjonalnie, skrót)}
% Pełny plik z \texttt{diary} służy raczej do pracy lokalnej. Do sprawka: wycinamy tylko istotne
% wiersze (bez promptów i bez linii z wywołaniami) do np.\ \texttt{wyniki\_skrot.txt}, potem:
% \verbatiminput{wyniki_skrot.txt}
\textit{(Tu: tylko jeśli potrzeba --- krótki fragment wyników, nie cały RAW z terminala.)}
